...
const DosageTracker: React.FC = () => {
    const [logs, setLogs] = useState<Log[]>([
        { id: 1, medication: 'Amoxicillin', dosage: '500mg', time: '08:00 AM', notes: 'Taken with food' },
        { id: 2, medication: 'Vitamin D', dosage: '1000IU', time: '09:30 AM', notes: '' },
        { id: 3, medication: 'Lisinopril', dosage: '10mg', time: 'Yesterday', notes: 'Routine check' },
    ]);
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
                ...
                >
                    ...
                    <div className="space-y-4 flex-grow overflow-y-auto pr-2 custom-scrollbar max-h-[600px]">
                        {logs.map((log) => (
                            <div key={log.id} className="group bg-white hover:bg-brand-bg border border-brand-subtle hover:border-brand-accent/50 rounded-2xl p-5 transition-all duration-300 flex items-start gap-5 shadow-sm hover:shadow-md">
                                <div className="w-14 h-14 rounded-2xl bg-brand-bg border border-brand-subtle flex items-center justify-center group-hover:scale-105 transition-transform shrink-0">
                                    <Pill className="w-6 h-6 text-brand-primary group-hover:text-brand-accent transition-colors" />
                                </div>
                                <div className="flex-1 min-w-0">
                                    <div className="flex justify-between items-start">
                                        <h4 className="font-bold text-lg text-brand-primary group-hover:text-brand-accent transition-colors truncate pr-2">{log.medication}</h4>
                                    ...
                                    <p className="text-sm text-brand-primary/60 mt-1 font-medium">Dosage: <span className="text-brand-primary">{log.dosage}</span></p>
                                    {log.notes && (
                                        <p className="text-xs text-brand-primary/50 mt-2 italic truncate">"{log.notes}"</p>
                                    )}
                                </div>
                    ...
                    <div className="bg-brand-primary rounded-[2rem] p-8 mt-8 flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden shadow-2xl">
                        ...
                        <button className="relative z-10 px-8 py-3 bg-white text-brand-primary font-bold rounded-full text-sm hover:scale-105 transition-transform whitespace-nowrap shadow-lg">
                            App Store
                        </button>
                    </div>
                </motion.div>
